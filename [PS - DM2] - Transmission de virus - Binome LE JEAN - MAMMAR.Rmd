...
```{r}
evolution_T_N <- function (simulateur = simulation1, generateur, pas=10, nbSamples=10, nbInfecté = 1) {
  library(ggplot2)
  set.seed(42);
  ...
  for(samples in 1:nbSamples) {
    Ni = samples * pas
    df2 = rbind(df2, data.frame(N = Ni, T_N = tail(simulateur(Ni, generateur, nbInfecté), 1)$Temps ))
  }
...
```{r}
courbe_epidemique(simulteur = simulation1, generateur = gene_arete_ligne, N = 500)
```
